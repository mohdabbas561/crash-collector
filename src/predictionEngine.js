'use strict';
// predictionEngine.js  v20-CLEAN
//
// ═══════════════════════════════════════════════════════════════════════════════
// CHANGELOG vs v19-FINAL
// ═══════════════════════════════════════════════════════════════════════════════
//
// MANDATE: Statistical integrity over prediction illusion.
//   The system operates under independent RNG (memoryless) conditions.
//   There is no real predictive signal in past outcomes.
//   Goal: probability estimation quality + trade selection efficiency.
//
// REMOVED (entirely):
//   ✗ Random Forest (rf)
...
//   ✗ Meta ensemble (meta)
//   ✗ All ML training logic, model caching, retraining triggers
//   ✗ ML library lazy-loaders (getRF, getCART, getSS, getNB)
//   ✗ Tier-2 background ML runner
//   ✗ enableMLTraining(), mlModelCache, srvModelCache
//   ✗ Signal amplification (introduces artificial inflation)
//   ✗ High-conviction window boost (gambler's fallacy dressed as ML)
//   ✗ Aggressive mode window widening (regime-driven, unreliable on IID)
//   ✗ Best-model direct-use override (selectBestModelDirect)
//   ✗ Temperature scaling on ensemble (adds overconfidence)
//
// KEPT AND STRENGTHENED:
//   ✓ GEO  — Laplace-smoothed geometric probability
//   ✓ BAY  — Bayesian recency blend (max 5% weight, no regime boost)
//   ✓ KM   — Kaplan-Meier empirical gap CDF
//   ✓ ENS  — BMA ensemble of geo+bay+km only
//   ✓ ENGINE — base engine (geo-style with timing)
//   ✓ PATTERN — pattern tab (unchanged)
//   ✓ Hybrid window placement (parametric + KM blend, timing correction)
//   ✓ 4-zone decision system (unchanged zones, tightened confidence)
//   ✓ Calibration (tightened: max correction +-15%, slower warmup)
//   ✓ BMA ensemble weights (slower decay, reduced volatility)
//   ✓ Rare engine (Pareto + KM tail blend, unchanged)
//   ✓ Timing feedback (early-rate window correction)
//   ✓ processEngine, getStatus, loadLockedMap, buildSavePayload (unchanged)
//   ✓ In-memory rounds cache, runPredictionEngine (simplified — no Tier2)
//
// IMPROVED:
//   • Regime influence: max factor capped at 0.06 (was ~0.25 effective)
//     Regime now provides a weak observational hint, not a probability driver.
//   • BAY recency weight: hard-capped at 0.05 (was up to 0.35 with regime boost)
//     Recency blend in IID conditions is noise; kept minimal for stability only.
//   • Calibration correction ratio: clamped to [0.88, 1.12] (was [0.80, 1.20])
//     Prevents overshooting on sparse data; +0.04 max shift per step (was +0.05)
//   • CAL_DECAY tightened: normal 0.05 (was 0.08), rare 0.02 (unchanged)
//     Slower EWMA = more stable calibration bins, less noise from individual outcomes
//   • CAL_MIN_SAMPLES raised to 20 (was 12)
//     Ensures bins have enough observations before any correction is applied
//   • BMA decay slowed: normal 0.03 (was 0.05), rare 0.015 (was 0.02)
//     Prevents single-model dominance spikes from short winning streaks
//   • Spread computed in probability space (linear) not logit space
//     Logit-space spread inflated differences near boundaries; linear is correct
//   • confPenalty from streak z-score: softened sigmoid (was steep at z=1.5)
//     Reduced gambler's fallacy influence on confidence score
//   • signalStrength: regime factor contribution removed (was 10% of score)
//
// RESULT:
//   • Fewer TAKE signals (higher threshold effective through tighter calibration)
//   • More stable probabilities across restarts
//   • ECE reduced by removing overcorrection
//   • Win rate target: 52-60% on TAKE trades
//   • No artificial performance gains
//
// DB FORMAT: UNCHANGED. No migration needed.
// DEPLOY: Replace predictionEngine.js, restart. No other changes required.
//
// ═══════════════════════════════════════════════════════════════════════════════

const ENGINE_VERSION = 'v20-CLEAN';
const {
...
console.log('[v20-CLEAN] Loaded — geo/bay/km/ens only · regime dampened · calibration tightened');
...
// Active stat models — geo/bay/km/ens only. All ML removed.
const STAT_MODELS = [
  { id: 'ens' },
  { id: 'geo' },
  { id: 'bay' },
...
// ── Calibration constants (tightened) ────────────────────────────────────────
...
// ── Decision engine constants — 4-zone system (UNCHANGED) ────────────────────
...
// ── Rare engine constants (UNCHANGED) ────────────────────────────────────────
...
// ── Apply regime to window and probability (dampened) ─────────────────────────
...
// ── Pareto tail smoothing for rare targets ────────────────────────────────────
...
function buildEnsemble(targetLabel, probGeo, probBay, probKm, cv) {
  ...
  // Spread in probability space (not logit)
...
// ── 4-zone signal decision (zones unchanged, amplification removed) ────────────
function computeSignalDecision(probW, confidence, spread, regimeFactor, regimeConfidence, isRandom, targetLabel, modelId, hits) {
  ...
  // No amplification. No high-conviction boost. Probability is what the math says.
...
function buildSignalResult(probW, signalStrength, recommendation, decisionReason) {
  ...
  return {
    ...
    amplifiedProbW: +probW.toFixed(4), // no amplification
...
function getStreakInfo(gs, maxWidth, isRare) {
  ...
  const confPenalty = Math.round(12 * sigmoid(z - 2.0)); // softened vs v19
...
function buildPrediction(rounds, targetMin, maxWidth, isRare, lastRoundId) {
  ...
  const gs = getGapStats(rounds, targetMin, lastRoundId);
  if (!gs) return null;
  const { hits, pGlobal, rateShifted, gapSinceLast, currentStreak, kmCDF, kmExpectedGap, cv } = gs;
  ...
  return { low: rLow, high: rHigh, expectedGap, opensIn: rLow, confidence, probW: +probW.toFixed(4), p: +p.toFixed(6), rateShifted, cusumNorm: gs.cusumNorm, streakStatus, currentStreak, z, gapSinceLast, hits, n: gs.n, regime: regime.regimeLabel, regimeFactor: regime.regimeFactor, regimeConfidence: regime.regimeConfidence, isRandom: gs.isRandom, maxAC: gs.maxAC, ...decision };
}
...
function buildGeo(gs, maxWidth, targetLabel, isRare, rounds, targetMin, engineGapSinceLast) {
  ...
  return { low: rLow, high: rHigh, expectedGap, opensIn: rLow, confidence, probW: +calibProbW.toFixed(4), rawProbW: +rawProbW.toFixed(4), p: +pGeo.toFixed(6), streakStatus, currentStreak, z, gapSinceLast, hits, n, rateShifted: gs.rateShifted, model: 'geo', regime: regime.regimeLabel, regimeFactor: regime.regimeFactor, regimeConfidence: regime.regimeConfidence, isRandom: gs.isRandom, maxAC: gs.maxAC, ...decision };
}

// ── BUILD: BAY ────────────────────────────────────────────────────────────────
function buildBay(gs, maxWidth, targetLabel, isRare, rounds, targetMin, engineGapSinceLast) {
  const { hits, n, pGlobal, pRecent, rateShifted, currentStreak, kmCDF, cv } = gs;
  const gapSinceLast = engineGapSinceLast ?? gs.gapSinceLast;
  ...
  const confidence                 = Math.max(20, betaConf(calibProbW, hits, null, regime.regimeConfidence, z) - confPenalty);
  const decision                   = computeDecision(calibProbW, confidence, null);
  return { low: rLow, high: rHigh, expectedGap, opensIn: rLow, confidence, probW: +calibProbW.toFixed(4), rawProbW: +rawProbW.toFixed(4), p: +pBay.toFixed(6), streakStatus, currentStreak, z, gapSinceLast, hits, n, rateShifted, model: 'bay', regime: regime.regimeLabel, regimeFactor: regime.regimeFactor, regimeConfidence: regime.regimeConfidence, isRandom: gs.isRandom, maxAC: gs.maxAC, ...decision };
}

// ── BUILD: KM ─────────────────────────────────────────────────────────────────
function buildKm(gs, maxWidth, targetLabel, isRare, rounds, targetMin, engineGapSinceLast) {
  const { hits, n, kmCDF, kmExpectedGap, currentStreak, cv } = gs;
  const gapSinceLast = engineGapSinceLast ?? gs.gapSinceLast;
  ...
  const confidence                 = Math.max(20, betaConf(calibProbW, hits, null, regime.regimeConfidence, z) - confPenalty);
  const decision                   = computeDecision(calibProbW, confidence, null);
  return { low: rLow, high: rHigh, expectedGap: kmExpectedGap, opensIn: rLow, confidence, probW: +calibProbW.toFixed(4), rawProbW: +rawProbW.toFixed(4), p: +rawProbW.toFixed(6), streakStatus, currentStreak, z, gapSinceLast, hits, n, rateShifted: gs.rateShifted, model: 'km', regime: regime.regimeLabel, regimeFactor: regime.regimeFactor, regimeConfidence: regime.regimeConfidence, isRandom: gs.isRandom, maxAC: gs.maxAC, ...decision };
}

// ── BUILD: ENS ────────────────────────────────────────────────────────────────
function buildEns(gs, maxWidth, targetLabel, isRare, rounds, targetMin, engineGapSinceLast) {
  const { hits, n, pGlobal, pRecent, rateShifted, kmCDF, kmExpectedGap, currentStreak, cv } = gs;
  const gapSinceLast = engineGapSinceLast ?? gs.gapSinceLast;
  ...
  return { low: rLow, high: rHigh, expectedGap: ensExpectedGap, opensIn: rLow, confidence, probW: +calibrated.toFixed(4), rawProbW: +ensProb.toFixed(4), p: +pGeo.toFixed(6), streakStatus, currentStreak, z, gapSinceLast, hits, n, rateShifted, model: 'ens', spread: +spread.toFixed(3), modelDisagreementScore, ensembleConfidence, regime: regime.regimeLabel, regimeFactor: regime.regimeFactor, regimeConfidence: regime.regimeConfidence, ensWeights: { geo: +adjWeights[0].toFixed(3), bay: +adjWeights[1].toFixed(3), km: +adjWeights[2].toFixed(3) }, isRandom: gs.isRandom, maxAC: gs.maxAC, ...decision };
}
...
function buildRare(gs, maxWidth, targetLabel, isRare, rounds, targetMin, engineGapSinceLast) {
  const { hits, n, pGlobal, currentStreak, kmCDF, kmExpectedGap, cv, p95, meanGap, stdGap } = gs;
  const gapSinceLast = engineGapSinceLast ?? gs.gapSinceLast;
  const regime = updateEngineRegime(rounds, targetMin, targetLabel, 'ens', gs, gs.isRandom);
...
function buildPatternPrediction(sortedRounds, targetMin) {
  ...
  let bestAC=0;
  for(let lag=1;lag<=Math.min(3,gaps.length-1);lag++){let cov=0;for(let i=lag;i<gaps.length;i++)cov+=(gaps[i-lag]-meanGap)*(gaps[i]-meanGap);const ac=varSum>0?cov/varSum:0;if(Math.abs(ac)>Math.abs(bestAC))bestAC=ac;}
  ...
  const conf=Math.max(25,Math.min(82,Math.round(32+Math.min(18,Math.log2(hits+1)*4)+absComposite*30+(agree-1)*6-(cv>1.5?8:cv>1.2?4:0))));
  ...
  for (const [label, pred] of Object.entries(lockedMap)) {
    ...
    out[label] = {
      ...
      eta: {
        ...
        regime:pred.regime??null, regimeFactor:pred.regimeFactor??null, regimeConfidence:pred.regimeConfidence??null,
        ensWeights:pred.ensWeights??null,
        isRandom:pred.isRandom??null, maxAC:pred.maxAC??null,
        ev:pred.ev??null, signalQuality:pred.signalQuality??null, risk:pred.risk??null,
        recommendation:pred.recommendation??null, decisionReason:pred.decisionReason??null,
        signalStrength:pred.signalStrength??null,
        finalProbUsed:pred.finalProbUsed??null, amplifiedProbW:pred.amplifiedProbW??null,
        tailProbability:pred.tailProbability??null, extremeGapScore:pred.extremeGapScore??null,
        rareEV:pred.rareEV??null, earlyWindow:pred.earlyWindow??null, lateWindow:pred.lateWindow??null,
        primaryWindow:pred.primaryWindow??null, alpha:pred.alpha??null, rareSignal:pred.rareSignal??null,
        n:pred.n??null,
      },
    };
  }
...
function loadLockedMap(dbRows) {
  ...
  for (const [label, pred] of Object.entries(dbRows)) {
    ...
    if (high - low + 1 > target.maxWidth) {
      console.log(`[engine] DISCARD stale wide window ${label}: saved ${high-low+1}r > max ${target.maxWidth}r`);
      continue;
    }
    map[label] = {
      ...
      spread: eta.spread??null, regime: eta.regime??null, regimeFactor: eta.regimeFactor??null,
      regimeConfidence: eta.regimeConfidence??null, ensWeights: eta.ensWeights??null,
  ...
  for (const t of TARGETS) {
    ...
    for (const id of ENGINE_IDS) {
      ...
      out[t.label][id] = {
        ...
        ece:              STAT_MODELS.some(m => m.id === id) ? (getECE(t.label, id) != null ? +getECE(t.label, id).toFixed(4) : null) : null,
      };
    }
  }
  return out;
...
function getTimingStats() {
  ...
  for (const t of TARGETS) {
    ...
    out[t.label] = { earlyRate: +earlyRate.toFixed(3), recentEarlyRate: +recentEarlyRate.toFixed(3), timingShiftFactor: +timingShiftFactor.toFixed(3), totalOutcomes: ts.totalCount, earlyOutcomes: ts.earlyCount, recentWindow: ts.earlyQueue.length, hasData };
  }
  return out;
}
...
let cachedRounds = [], cachedRoundsLastId = 0;
...
// ── Main — single tier, no ML background thread ───────────────────────────────
async function runPredictionEngine() {
  try {
    ...
    console.log(`[engine v20-CLEAN] done in ${Date.now()-t1Start}ms`);
