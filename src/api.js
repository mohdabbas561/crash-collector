...
const {
  ...
  getCrashRounds, saveCrashRounds, saveTowerPredictionHistory, clearCrashRounds, clearAllCrashRounds, getCrashSummary, getCrashDashboard, touchCrashSite,
...
app.post('/vault/session', requireDatabase, rateLimit(20), async (req, res) => {
  try {
    const password = normalizeSecretValue(
      req.body?.password ||
      req.body?.secret ||
      req.body?.token ||
      req.query?.password ||
